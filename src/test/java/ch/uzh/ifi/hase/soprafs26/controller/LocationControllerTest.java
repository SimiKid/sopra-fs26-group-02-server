...
class LocationControllerTest {
    ...
    void getLocationForGame_validCode_returnsLocationName() throws Exception {
        given(gameSessionService.getLocationByCode("ABC123")).willReturn("ZURICH");

        mockMvc.perform(get("/games/ABC123/location"))
            .andExpect(status().isOk())
            .andExpect(content().string("ZURICH"));

        verify(gameSessionService).getLocationByCode("ABC123");
    }

    @Test
    void getLocationForGame_unknownCode_returnsNotFound() throws Exception {
        given(gameSessionService.getLocationByCode("UNKNOWN"))
